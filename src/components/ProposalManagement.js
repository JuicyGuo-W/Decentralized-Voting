...
export default function ProposalManagement() {
    ...
    const [maxVotingNum, setMaxVotingNum] = useState('')
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (typeof window.ethereum !== 'undefined') {
                ...
                const tx = await contract.addProposal(description, startTimestamp, endTimestamp, maxVotingNum)
                console.log('tx:', tx);
                await tx.wait()
            ...
            <VStack spacing={6}>
                <FormControl>
                    <form onSubmit={handleSubmit}>
                        <VStack spacing={4}>
                            ...
                            <FormControl>
                                <FormLabel>Max votes per voter</FormLabel>
                                <Input
                                    value={maxVotingNum}
                                    onChange={(e) => setMaxVotingNum(e.target.value)}
                                    placeholder="Enter max votes per number"
                                />
                            </FormControl>
                            <Button
                                type="submit"
                ...
                {proposal && (
                    <Box borderWidth={1} borderRadius="lg" p={4} width="100%">
                        ...
                        <Text>Max Votes per voter: {proposal.maxVotingNum.toString()}</Text>
